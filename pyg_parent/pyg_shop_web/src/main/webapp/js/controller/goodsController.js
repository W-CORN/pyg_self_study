...
app.controller('goodsController', function ($scope, $controller, goodsService, uploadService, itemCatService, typeTemplateService, $location) {
    ...
    //规格列表显示
    $scope.ItemSpecList = function ($event) {
        //定义模板
        ...
        for (var i = 0; i < item.length; i++) {
            //获取名称
            var specName = item[i].attributeName;
            //获取规格属性
            var specValue = item[i].attributeValue;
            var newList = [];
            //遍历模板
            for (var j = 0; j < $scope.entity.itemList.length; j++) {
                ...
                //遍历规格属性
                for (var x = 0; x < specValue.length; x++) {
                    //克隆SKU
                    var newSKU = JSON.parse(JSON.stringify(oldSKU));
                    newSKU.spec[specName] = specValue[x];
                    newList.push(newSKU);
                }
            }
    ...
    //规格启用后再次未启用后列表取消
    $scope.$watch("entity.tbGoods.isEnableSpec=='0'", function () {
        $scope.entity.itemList = null;
        $scope.entity.tbGoodsDesc.specificationItems = [];
    });
    $scope.$watch("entity.tbGoodsDesc.itemImages==null",function () {
    ...
    $scope.findIsMarketable=['已下架','已上架'];
    //上下架
