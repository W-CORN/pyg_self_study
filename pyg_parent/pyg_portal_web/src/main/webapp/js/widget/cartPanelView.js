var cartPanelView = {
    ...
    setup : function (callback) {
        ...
        $('.tbar-seckill-item').hover(function (){ $(this).find('.p-del').show(); },function(){ $(this).find('.p-del').hide(); });
        $('.jth-item').hover(function (){ $(this).find('.add-seckill-button').show(); },function(){ $(this).find('.add-seckill-button').hide(); });
        ...
        cartPanelView.cartCellTemplate = $("#tbar-seckill-item-template").html();
    ...
    fillCart : function(dataJSON) {
        ...
        $("#seckill-list").html(rowsHtml);
        // 购物车小计
        $("#seckill-number").html(dataJSON.totalQuantity);
        $("#seckill-sum").html(String.format("¥{0}",dataJSON.totalPrices));
        // 侧栏 购物件数
        $("#tab-sub-seckill-count").html(dataJSON.totalQuantity);
    }
};
